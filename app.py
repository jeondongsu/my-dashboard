...
st.set_page_config(layout="wide", page_title="통합 지휘소 V10.0")
...
# 💡 [핵심 추가] 텔레그램 발송 내역을 기록할 메모리 창고
if 'alert_logs' not in st.session_state: st.session_state.alert_logs = []

# 💡 외부 엑셀(CSV) 파일을 1초 만에 읽어와 메모리에 박아두는 로더
@st.cache_data
def load_region_codes():
    try:
        df = pd.read_csv("region_codes.csv", dtype={"code": str})
        return dict(zip(df["code"], df["region"]))
    except Exception:
        return {"11500": "서울특별시 강서구", "41171": "경기도 안양시 만안구"}
...
# --- [메인 화면] 작전 제어판 ---
st.title("🏢 SD 전용 무인 감시 지휘소 (V10.0)")
...
with tab2:
    ...
    # --- 💡 [핵심 구역] 자동 감시 및 블랙박스(로그) 시스템 ---
    if st.checkbox("🔄 24시간 무인 자동 감시 작동 - 1분마다"):
        st_autorefresh(interval=60000, limit=None, key="auto_refresh")
        
        kst = datetime.timezone(datetime.timedelta(hours=9))
        now = datetime.datetime.now(kst)
        log_time = now.strftime('%Y-%m-%d %H:%M:%S') # 로그에 찍힐 KST 시간
        ...
        if is_weekday and is_market_hours:
            ...
            if dw_price > 0 and dw_price <= dw_buy_target and not st.session_state.dw_buy_fired:
                ...
                st.session_state.alert_logs.insert(0, f"[{log_time}] {msg}") # 로그 저장 (가장 최신이 위로)
